...
function isFrontendDev(languages) {
  return new Promise(function (resolve, reject) {
    if (!languages.includes("html")) {
      reject("You are not Frontend deverloper");
    }
    setTimeout(() => {
      resolve("You are frontend deverloper");
    }, 1000);
  });
}
const dev1 = isFrontendDev(["html", "css"]);
const dev2 = isFrontendDev(["css"]);
// devAll báo lỗi khi 1 cái không thỏa mãn
...
// thất bại hay thành công đều trả về

// try catch

function isFrontendDev2(languages) {
  if (!languages.includes("html")) {
    throw new Error("You are not Frontend deverloper");
  }
  return new Promise(function (resolve, reject) {
    setTimeout(() => {
      resolve("You are frontend deverloper");
